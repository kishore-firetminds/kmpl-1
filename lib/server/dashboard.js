...
export const DEFAULT_TEAM_OWNER_BUDGET = 100000;
...
export function calculateTeamSummary(teamOwner, players) {
  const budget = Number(teamOwner?.auctionBudget || DEFAULT_TEAM_OWNER_BUDGET);
  const spentPoints = players.reduce((sum, player) => sum + Number(player?.playerPoints || 0), 0);
  return {
    budget,
    spentPoints,
    remainingPoints: budget - spentPoints,
