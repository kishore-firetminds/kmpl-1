...
export function toPlayerRow(player) {
  return {
    ...
    assigned_team_owner_id: player.assignedTeamOwnerId || null,
    assigned_team_name: player.assignedTeamName || null,
    player_points: Number(player.playerPoints || 0),
...
export function fromTeamOwnerRow(row) {
  return {
    ...
    auctionBudget: Number(row.auction_budget || 100000),
    password: row.password,
    feePaid: row.fee_paid,
    paymentRef: row.payment_ref,
    registeredAt: row.registered_at
...
export function toTeamOwnerRow(owner) {
  return {
    ...
    auction_budget: Number(owner.auctionBudget || 100000),
